...
//
	...
	while(1)
	{
		...
		if(!(GPIOB->IDR & 1) && !buttonModeProcessed && ((TIM1->CNT - buttonModeLastDebounceTime) > 50) && buttonModeLastDebounceTime != 0)
		{
			...
			if(alarmClockState == ALARM_CLOCK_ALARM || alarmClockState == ALARM_CLOCK_SET_ALARM)
			{
			...
			else if(alarmClockState == ALARM_CLOCK_NO_ALARM || alarmClockState == ALARM_CLOCK_SET)
			{
				uint8_t hoursTen,hoursOne,minsTen,minsOne;
